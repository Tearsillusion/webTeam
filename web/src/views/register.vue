<template>
	<div class="register">
		...
		<div class="web-team-login-right-center">
			...
			<el-input v-model="data.nickName" @keypress.native.enter="loginClick" type="text" placeholder="请输入昵称"  />
			...
			<el-button type="primary" @click="registerClick" :loading="loading">{{data.registerText}}</el-button>
		</div>
...
<script lang="ts" setup>
	...
	import {HttRegister} from '../public/http'
	...
	const registerClick = ()=>{
		if(!data.email){
			...
			HttRegister({
				...
				loading.value = false
				data.registerText = "注册"
				if(res.code === 200){
					ElMessage({
					    message: '恭喜您注册成功，赶快登录吧!',
					    type: 'success',
					})
					router.push('/login')
